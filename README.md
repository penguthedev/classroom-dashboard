...
![Gemini](https://img.shields.io/badge/GenAI-Google%20Gemini-8E75B2?logo=googlegemini&logoColor=white)

[Features](#-features) · [Architecture](#-architecture) · [Quick start](#-quick-start) · [Configuration](#-configuration) · [AI assistant](#-how-the-ai-assistant-works) · [API](#-api-overview) · [Deployment](#-deployment)
...
```mermaid
flowchart LR
    U["👤 User<br/>Browser"] --> FE["React SPA<br/>Netlify CDN"]
    FE -- "REST + JWT" --> API["FastAPI<br/>Render"]
    FE -- "signed upload" --> CDN[("Cloudinary")]
    API --> DB[("PostgreSQL<br/>Neon")]
    API -- "function calling" --> LLM["Google Gemini"]
    API --> CDN
    API -. "optional" .-> SMTP["SMTP"]
```

The architecture diagram, API sequence diagrams and CI/CD pipeline are documented in detail in **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**.
...
- A Gemini API key from [Google AI Studio](https://aistudio.google.com/apikey) — optional, only needed for the assistant
...
<details>
<summary><b>Choosing a Gemini model</b></summary>
...
Both platforms auto-deploy on push to `main`. Migrations are run manually before a deploy. See **[DEPLOYMENT.md](DEPLOYMENT.md)** for the step-by-step guide and **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#4-cicd-pipeline)** for the pipeline diagram.
...
2. Add an Alembic migration for any model change: `python -m alembic revision --autogenerate -m "describe change"`
3. Run the backend check scripts plus `npm run lint` and `npm run build`
